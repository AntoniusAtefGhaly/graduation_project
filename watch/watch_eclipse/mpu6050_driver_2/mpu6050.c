 ...
 */
void step_counter() {
	avgAcc = gyro_sumFF;		//1 stage filter
	avgAcc = gyro_sumFF; 		//2 stage filter

	if (avgAcc >= step_threshold && flag == 0 ){
		steps_number = steps_number + 1;
		flag=1;
	}
	else if((avgAcc >=step_threshold) && (flag == 1)) {
	}
	else{
		flag =0;
	}
...
void fallen_detection(void)
{
	...
	int fallen_parameter;
	fallen_parameter = (int)Adsvm * 100;
	if ((fallen_parameter >= fallen_threshold) &&( flag == 0 )){
	...
	else if((fallen_parameter >= fallen_threshold) && (flag == 1)) {
	}
	else{
		flag =0;
	}
	pre_Xa = Ya ;                                             //save A(t) to A(t-1)
	pre_Ya = Ya ;
	pre_Za = Za ;
}
...
void check_motion_states(void){
	...
	if(count==200){
		...
		if( steps_number2 > (steps_number1+1)){
			motion_states =1;
		}
		else
		{
